...
<div id="screen-dashboard" class="screen">
  ...
  <div class="tabnav">
    ...
    <button class="tabtop" id="tab-btn-carto" onclick="switchTab('cartographie',this)">🗺️ Cartographie</button>
  </div>
  ...
  <div class="page-content" style="padding-top:104px; max-width:960px; margin:0 auto; padding-left:14px; padding-right:14px; padding-bottom:60px;">
    ...
    <div class="tabcontent" id="tab-equipes">
      ...
      <div class="card" style="padding:0;overflow:hidden;margin-bottom:14px;">
        <div style="padding:10px 14px;background:linear-gradient(135deg,var(--c1),var(--c2));color:#fff;display:flex;align-items:center;justify-content:space-between;">
          ...
          <button onclick="switchTab('cartographie',document.querySelector('[onclick*=cartographie]'));" style="background:rgba(255,255,255,0.18);border:none;border-radius:8px;padding:4px 10px;font-size:11px;font-weight:700;color:#fff;cursor:pointer;">Agrandir →</button>
        </div>
        <!-- Légende compacte -->
    ...
    <div class="tabcontent" id="tab-cartographie">
      ...
      <!-- Carte -->
      <div class="card" style="padding:0;overflow:visible;margin-bottom:14px;">
...
function initCarto() {
  ...
  // Forcer les dimensions avant init Leaflet
  el.style.height = '420px';
  el.style.width = '100%';
  el.style.display = 'block';
  // Petit délai pour que le DOM applique les styles
  setTimeout(function() {
    try {
      cartoMap = L.map('carto-map', {
        zoomControl: true,
        scrollWheelZoom: true,
        preferCanvas: true
      }).setView([34.028, -5.003], 13);
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      ...
      setTimeout(function(){ cartoMap.invalidateSize(); updateCartoMap(); }, 300);
      setTimeout(function(){ cartoMap.invalidateSize(); }, 800);
    } catch(e) { console.error('initCarto error:', e); }
  }, 100);
}
...
function initZoneMap() {
  ...
  setTimeout(function() {
    try {
      ...
      setTimeout(function(){ zoneMap.invalidateSize(); updateZoneMap(); }, 300);
      setTimeout(function(){ zoneMap.invalidateSize(); }, 800);
    } catch(e) { console.error('initZoneMap error:', e); }
  }, 100);
}
...
function switchTab(name, btn) {
  ...
  if (name === 'equipes') {
    renderEquipes(); renderZoneChips();
    // Tab est maintenant visible — init ou refresh la mini-carte
    setTimeout(function() {
      initZoneMap();
      setTimeout(function(){ if(zoneMap) zoneMap.invalidateSize(); }, 200);
      setTimeout(function(){ if(zoneMap) { zoneMap.invalidateSize(); updateZoneMap(); } }, 600);
    }, 50);
  }
  if (name === 'cartographie') {
    // Tab est maintenant visible — init ou refresh la grande carte
    setTimeout(function() {
      initCarto();
      setTimeout(function(){ if(cartoMap) cartoMap.invalidateSize(); }, 200);
      setTimeout(function(){ if(cartoMap) { cartoMap.invalidateSize(); updateCartoMap(); } }, 600);
    }, 50);
  }
